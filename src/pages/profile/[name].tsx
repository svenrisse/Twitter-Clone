...
export default function UserPage() {
  ...
  const comments =
    countComments && countComments > 0 ? (
      userData?.tweet.map((tweet) => {
        return <Tweet key={tweet.id} tweet={tweet} />;
      })
    ) : (
      <span>This user has no Comments yet!</span>
    );
              ...
              {active === "follows" && (
                <>
                  {userData._count.follows >= 1 ? (
                    <div className="mt-12 mb-20 h-max w-11/12 rounded-xl border-l-2 border-r-2 border-t-2 border-slate-400 md:w-3/4 lg:w-1/2 2xl:w-5/12">
                      {follows}
                    </div>
                  ) : (
                    <span className="flex justify-center pt-12 text-gray-600">
